...
class App extends Component {
    ...
    trackScrolling = (e, query = "") => {
        ...
        if (query != "") {
            ...
            link = `https://api.giphy.com/v1/gifs/search?q=${query}&offset=${newOffset}&limit=${ITEM_LIMIT}&api_key=${API_KEY}`;
        } else {
            link = `https://api.giphy.com/v1/gifs/trending?offset=${newOffset}&limit=${ITEM_LIMIT}&api_key=${API_KEY}`;
        }
    ...
    performSearch = (query) => {
        ...
        axios
            .get(
                `https://api.giphy.com/v1/gifs/search?q=${query}&limit=${ITEM_LIMIT}&api_key=${API_KEY}`
            )
            .then((response) => {
                // handle success
                this.setState({
                ...
                window.addEventListener("scroll", (e) =>
                    this.trackScrolling(e, this.state.searchFilter)
                );
            })
            .catch(function (error) {
                // handle error
    ...
    trendingSearch = () => {
        axios
            .get(
                `https://api.giphy.com/v1/gifs/trending?limit=${ITEM_LIMIT}&api_key=${API_KEY}`
            )
            .then((response) => {
                // handle success
                this.setState({
